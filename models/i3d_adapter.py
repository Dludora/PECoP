...
#This class (ConvAdapter) was originally written by the authors of:
#https://arxiv.org/abs/2208.07463 for the 2D CNNs and modified by us to be applied on 3DC NNs
    ...
    def __init__(self, inplanes, outplanes, width, 
        ...
        self.conv1 = nn.Conv3d(inplanes, width, kernel_size=kernel_size, stride=stride, groups=groups, padding=padding, dilation=int(dilation))
  
        self.act = act_layer()
        self.conv2 = nn.Conv3d(width, outplanes, kernel_size=1, stride=1)


  
        self.se = nn.Parameter(1.0 * torch.ones((1, outplanes, 1, 1,1)), requires_grad=True)
...
class Mixed(torch.nn.Module):
    def __init__(self, in_channels, out_channels, adapter = True):
        ...
        if self.adapter:
            ...
            width = out_channels[0] + out_channels[2]+ out_channels[4] + out_channels[5]    
